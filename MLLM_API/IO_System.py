...
try:
    from MLLM_API.vLLM_API import generate_with_vLLM_model
except:
    pass

try:
    from MLLM_API.OpenAI_API import generate_n_with_OpenAI_model
except:
    pass

try:
    from MLLM_API.transformers_API import generate_with_transformers_model
except:
    pass
...
class IO_System:
    ...
    def generate(self, model_input, max_tokens: int, num_return: int, stop_tokens=None):
        if isinstance(model_input, str):
            if self.api == "vllm":
                ...
                self.token_counter += 0
            elif self.api == "transformers":
                raise NotImplementedError(f"The transformers API requires a fixed template as input; see MLLM_API/transformers_API.py for details")
            elif self.api == "debug":
            ...
            if self.api == "vllm":
                ...
                transformers_response = generate_with_transformers_model(
                    model=self.model,
                ...
                io_output_list = transformers_response
                self.call_counter += num_return
                self.token_counter += sum(
                    [
                        len(output)
                        for output in transformers_response
                    ]
                )
            elif self.api == "debug":
                io_output_list = [
